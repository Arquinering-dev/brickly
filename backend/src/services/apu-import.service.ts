...
export async function importAPU(parsed: ParsedAPU): Promise<ImportAPUSummary> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const allInsumos = await tx.insumo.findMany({ select: { id: true, codigo: true } });
    allInsumos.forEach((i) => insumoMap.set(i.codigo, i.id));

    const partidaMap = new Map<string, string>();
    const allPartidas = await tx.partida.findMany({ select: { id: true, codigo: true } });
    allPartidas.forEach((p) => partidaMap.set(p.codigo, p.id));
